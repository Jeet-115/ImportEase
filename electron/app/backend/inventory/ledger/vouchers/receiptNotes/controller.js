...
import { findByTrackingNo } from "../../tracking/model.js";

const asyncHandler = (fn) => async (req, res, next) => {
  try {
    await fn(req, res, next);
  } catch (error) {
    console.error("[ReceiptNotesController] Error:", error);
...
export const createReceiptNote = asyncHandler(async (req, res) => {
  ...
  // If trackingNo provided, validate it exists
  if (trackingNo) {
    const tracking = await findByTrackingNo(companyId, trackingNo);
    if (!tracking) {
      return res.status(400).json({ message: `Tracking number ${trackingNo} not found` });
    }
  }

  const voucherId = `RN-${voucherNo}`;
  const voucher = {
    voucherId,
    voucherNo,
    date,
  ...
  // Create transactions (stock comes in)
  ...
  await saveVoucherWithTransactions(companyId, "receiptNotes", voucher, transactions);
