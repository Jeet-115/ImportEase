...
const asyncHandler = (fn) => async (req, res, next) => {
  try {
    ...
    console.error("[RejectionsOutController] Error:", error);
...
export const createRejectionOut = asyncHandler(async (req, res) => {
  const { companyId } = req.params;
  const { voucherNo, date, partyId, items, remarks } = req.body;
  ...
  const voucherId = `REJ-OUT-${voucherNo}`;
  const voucher = {
    voucherId,
    voucherNo,
    date,
  ...
  const transactions = items.map((item) =>
    createTransaction(
      ...
      {
        ...item,
        qty: -Math.abs(item.qty), // Always negative for rejection out
      }
    )
  );

  await saveVoucherWithTransactions(companyId, "rejectionsOut", voucher, transactions);
