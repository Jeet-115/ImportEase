...
const asyncHandler = (fn) => async (req, res, next) => {
  try {
    ...
    console.error("[RejectionsInController] Error:", error);
...
export const createRejectionIn = asyncHandler(async (req, res) => {
  ...
  const voucherId = `REJ-IN-${voucherNo}`;
  const voucher = {
    voucherId,
    voucherNo,
    date,
  ...
  const transactions = items.map((item) =>
    createTransaction(
      ...
      {
        ...item,
        qty: Math.abs(item.qty), // Always positive for rejection in
      }
    )
  );

  await saveVoucherWithTransactions(companyId, "rejectionsIn", voucher, transactions);
