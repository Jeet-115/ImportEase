...
import { findByTrackingNo, create as createTracking } from "../../tracking/model.js";
...
const asyncHandler = (fn) => async (req, res, next) => {
  try {
    ...
    console.error("[DeliveryNotesController] Error:", error);
...
export const createDeliveryNote = asyncHandler(async (req, res) => {
  ...
  // Create tracking number if provided
  let finalTrackingNo = trackingNo;
  if (trackingNo && !(await findByTrackingNo(companyId, trackingNo))) {
    await createTracking(companyId, {
      trackingNo,
      sourceVoucher: "DELIVERY_NOTE",
      targetVoucher: "SALES",
      status: "OPEN",
  ...
  const voucher = {
    ...
    items,
    trackingNo: finalTrackingNo || null,
  ...
  const transactions = items.map((item) =>
    createTransaction(
      ...
      {
        ...item,
        qty: -Math.abs(item.qty), // Always negative for delivery
      }
    )
  );

  await saveVoucherWithTransactions(companyId, "deliveryNotes", voucher, transactions);
