...
const initialFormState = {
  ...
  parentGodownId: "",
  allowStorage: true,
  isThirdPartyOurStock: false,
  isThirdPartyTheirStock: false,
  address: "",
};
...
const Godowns = () => {
  ...
  const loadGodowns = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await getGodowns(companyId);
      setGodowns(data || []);
    } catch (error) {
      console.error("Failed to load godowns:", error);
      setStatus({
        type: "error",
        message: "Unable to load godowns. Please try again.",
      });
    } finally {
      setLoading(false);
    }
  ...
  const buildHierarchy = (items, parentId = null) => {
    return items
      .filter((item) => item.parentGodownId === parentId)
      .map((item) => ({
        ...item,
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const payload = {
        ...
        parentGodownId: formData.parentGodownId || undefined,
        allowStorage: formData.allowStorage ?? true,
        isThirdPartyOurStock: formData.isThirdPartyOurStock || false,
        isThirdPartyTheirStock: formData.isThirdPartyTheirStock || false,
        address: formData.address?.trim() || "",
      };

      if (editingId) {
        await updateGodown(companyId, editingId, payload);
        setStatus({ type: "success", message: "Godown updated successfully." });
      } else {
        await createGodown(companyId, payload);
        setStatus({ type: "success", message: "Godown created successfully." });
      }
      ...
      setStatus({
        ...
        message:
          ...
          "Unable to save godown. Please retry.",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const handleEdit = (godown) => {
    ...
    setFormData({
      name: godown.name || "",
      parentGodownId: godown.parentGodownId || "",
      allowStorage: godown.allowStorage ?? true,
      isThirdPartyOurStock: godown.isThirdPartyOurStock || false,
      isThirdPartyTheirStock: godown.isThirdPartyTheirStock || false,
  ...
  const handleDelete = async (id, name) => {
    ...
    setSubmitting(true);
    try {
      await deleteGodown(companyId, id);
      setStatus({ type: "success", message: "Godown deleted successfully." });
      await loadGodowns();
    } catch (error) {
      ...
      setStatus({
        ...
        message:
          error?.response?.data?.message ||
          "Unable to delete godown. Please retry.",
      });
    } finally {
      setSubmitting(false);
    }
  ...
  const hierarchy = buildHierarchy(godowns);
  const flatGodowns = flattenHierarchy(hierarchy);
  const filteredGodowns = flatGodowns.filter((godown) =>
      ...
      <section className="mx-auto max-w-6xl space-y-5">
        ...
        >
          ...
          <p className="text-sm text-slate-600">
            Manage hierarchical storage locations (Region → Warehouse → Rack → Bin).
          </p>
        </motion.header>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className="text-sm text-slate-700">
                  Parent Godown
                  <select
                    value={formData.parentGodownId}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        parentGodownId: e.target.value,
                      })
                    }
                    className="mt-2 w-full rounded-xl border border-amber-200 bg-white px-4 py-2 text-sm shadow-sm focus:border-amber-400 focus:outline-none focus:ring-2 focus:ring-amber-100"
                    disabled={submitting || readOnly || editingId === formData.parentGodownId}
                  >
              ...
              <div className="space-y-3">
                ...
                <label className="flex items-center gap-2 text-sm text-slate-700">
                  <input
                    type="checkbox"
                    checked={formData.isThirdPartyOurStock}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        isThirdPartyOurStock: e.target.checked,
                      })
                    }
                    className="rounded border-amber-200 text-amber-500 focus:ring-amber-100"
                    disabled={submitting || readOnly}
                  />
                  Third Party (Our Stock)
                </label>
                <label className="flex items-center gap-2 text-sm text-slate-700">
                  <input
                    type="checkbox"
                    checked={formData.isThirdPartyTheirStock}
                    onChange={(e) =>
                      setFormData({
                        ...formData,
                        isThirdPartyTheirStock: e.target.checked,
                      })
                    }
                    className="rounded border-amber-200 text-amber-500 focus:ring-amber-100"
                    disabled={submitting || readOnly}
                  />
                  Third Party (Their Stock)
                </label>
              </div>
            ...
            <div className="flex items-center gap-3">
              <span className="text-sm text-slate-600">
                {loading
                  ? "Loading..."
                  : searchTerm
                  ? `${filteredGodowns.length} of ${godowns.length} godowns`
                  : `${godowns.length} godowns`}
              </span>
              <button
                type="button"
                onClick={loadGodowns}
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div
                          ...
                          style={{ paddingLeft: `${godown.level * 20}px` }}
                        >
                          {godown.level > 0 && "└ "}
                          {godown.name}
                        </div>
