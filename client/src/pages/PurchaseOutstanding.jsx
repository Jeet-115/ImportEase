...
const PurchaseOutstanding = () => {
  ...
  const loadOutstanding = async () => {
    ...
    try {
      ...
      // Filter for purchase vouchers only
      const purchaseOutstanding = (data || []).filter(
        (item) => item.voucherType === "PURCHASE" || item.voucherType === "purchase"
      );
      setOutstanding(purchaseOutstanding);
    } catch (error) {
      ...
      <section className="mx-auto max-w-6xl space-y-5">
        ...
        >
          ...
          <h1 className="text-3xl font-bold text-slate-900">
            {company?.companyName || "Purchase Outstanding"}
          </h1>
          <p className="text-base text-slate-600">
            View party-wise outstanding purchase bills
          </p>
        </motion.header>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Party-wise outstanding purchase bills
                </p>
              </div>
            </div>
            <button
